...
# MEASURED: packer validate accepted this template on 2026-09-24, with
# Packer 1.16.1, qemu plugin 1.1.6 and vagrant plugin 1.1.7; this
# project's CI re-validates it on every push, so a newer 1.x plugin may
# have validated it since. The values are the machine vmavs installs and
# boots. No Packer build has ever run from it, so whether the drive
# mapping and the SSH forward work in a real build is REASONED.
#
