...
| **`bats`, `shellcheck`, `python3`, `dmg2img`, `sgdisk`, `mkfs.hfsplus`** | not pinned | ❌ **not ingredients.** They are build-host tools: none of their bytes reaches an image. `bin/run-tests.sh` and `bin/preconditions.sh` check for what they need | n/a |
