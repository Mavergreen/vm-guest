...
| G15 | Firmware is passed with `-bios`, not as a pflash CODE/VARS pair, so **EFI variables do not persist** | P1 onward | A consequence of using the reference OVMF build. A host using its own split OVMF would get variable persistence back — P3 should check whether that changes OpenCore's behaviour. |
