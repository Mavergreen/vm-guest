...
| Compiler | `gcc` 13.3.0 (Ubuntu `13.3.0-6ubuntu2~24.04.1`), target `x86_64-linux-gnu`. **Defaults to `-std=gnu17`** — which is why the C23 breakage in G22 was invisible here. Probe it with `boot/build-opencore.sh --compiler` |
