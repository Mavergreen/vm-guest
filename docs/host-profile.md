...
| G21 | ~~**`kvm.ignore_msrs=1` is required.**~~ **REFUTED 2026-09-21 on `squirrel-zapper`**, which runs `ignore_msrs=N`: a guest installed (1332 s), booted without installer media, and answered SSH. The setting was applied here on 2026-09-17 on the authority of Somlo and OSX-KVM, and never tested without — for two phases it sat in §3 as a host state change rather than in §4 as a hypothesis, which is exactly how it escaped falsification. **Stop asking users for this `sudo` step** unless something shows it is needed; scope of the refutation is this CPU (Broadwell) and QEMU 11.1.1. | P1 onward | Worth noting what nearly hid it a second time: the entry existed in this ledger for a day with no verdict function in `bin/triangulate.sh`, so the run that settled it did not mention it. A hypothesis the harness cannot report on is not being tested by the harness. |
