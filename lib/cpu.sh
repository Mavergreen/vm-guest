...
Nehalem	BOOTED	2026-09-21, pet-power-plant (QEMU 8.2.2, -accel kvm): a guest installed under the default line booted on this one, answered SSH in 40 s, reported a cpubrand of Intel Core i7 9xx, Nehalem Class Core i7, with POPCNT added to its feature list, and hashed 64 MiB correctly. No guest has been INSTALLED on it. It is the first model with SSE4.2, so it is the model the default line is really approximating, and it is also the first with EPT -- docs/host-profile.md G18 and decisions/0005 both wait on that, and this row is now one step closer to answering them.
